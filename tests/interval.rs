use eos::{Date, Interval, ext::IntervalLiteral, Time, Error};
...
fn out_of_bounds_wrapping_time() -> Result<(), Error> {
    assert_eq!(Time::new(23, 59, 59)? + Interval::from_seconds(i64::MAX), Time::new(23, 59, 59)?);
    assert_eq!(Time::new(23, 59, 59)? - Interval::from_seconds(i64::MAX), Time::new(23, 59, 59)?);
    Ok(())
}
...
fn random_double_units_to_dates() -> Result<(), Error> {
    assert_eq!(Date::new(806, 2, 28)? - ((-7).weeks() + (-90).months()), Date::new(813, 10, 16)?);
    assert_eq!(Date::new(391, 11, 30)? + ((-7).weeks() + 22.years()), Date::new(413, 10, 12)?);
    assert_eq!(Date::new(1925, 7, 31)? + (56.weeks() + 51.months()), Date::new(1930, 11, 27)?);
    assert_eq!(Date::new(1423, 2, 28)? - ((-37).months() + (-51).years()), Date::new(1477, 3, 28)?);
    assert_eq!(Date::new(66, 11, 30)? + (-50).weeks(), Date::new(65, 12, 15)?);
    assert_eq!(Date::new(2020, 3, 31)? - 12.months(), Date::new(2019, 3, 31)?);
    assert_eq!(Date::new(46, 7, 31)? + ((-47).days() + (-41).months()), Date::new(43, 1, 12)?);
    assert_eq!(Date::new(987, 7, 31)? - (67.months() + (-26).days()), Date::new(982, 1, 26)?);
    assert_eq!(Date::new(1831, 4, 30)? - (-11).months(), Date::new(1832, 3, 30)?);
    assert_eq!(Date::new(812, 1, 31)? + ((-34).months() + 11.years()), Date::new(820, 3, 31)?);
    assert_eq!(Date::new(1194, 5, 31)? - ((-42).years() + (-79).days()), Date::new(1236, 8, 18)?);
    assert_eq!(Date::new(1943, 8, 31)? + (-80).days(), Date::new(1943, 6, 12)?);
    assert_eq!(Date::new(1609, 5, 31)? + 13.years(), Date::new(1622, 5, 31)?);
    assert_eq!(Date::new(614, 7, 31)? - ((-52).days() + (-66).years()), Date::new(680, 9, 21)?);
    assert_eq!(Date::new(170, 4, 30)? + 24.days(), Date::new(170, 5, 24)?);
    assert_eq!(Date::new(488, 2, 29)? + (12.weeks() + 96.months()), Date::new(496, 5, 23)?);
    assert_eq!(Date::new(1349, 10, 31)? + ((-90).days() + (-32).weeks()), Date::new(1348, 12, 21)?);
    assert_eq!(Date::new(1942, 7, 31)? + 53.weeks(), Date::new(1943, 8, 6)?);
    assert_eq!(Date::new(1095, 7, 31)? - (22.weeks() + 53.days()), Date::new(1095, 1, 5)?);
    assert_eq!(Date::new(539, 4, 30)? + (3.months() + 48.weeks()), Date::new(540, 6, 30)?);
    Ok(())
}
#[test]
...
fn random_double_units_to_times() -> Result<(), Error> {
    assert_eq!(Time::new(9, 16, 42)? + (86.hours() + 17.minutes()), Time::new(23, 33, 42)?);
    assert_eq!(Time::new(15, 13, 14)? + (-47).hours(), Time::new(16, 13, 14)?);
    assert_eq!(Time::new(17, 39, 18)? + 96.hours(), Time::new(17, 39, 18)?);
    assert_eq!(Time::new(0, 40, 43)? + (0.seconds() + (-46).hours()), Time::new(2, 40, 43)?);
    assert_eq!(Time::new(12, 48, 19)? + (-27).seconds(), Time::new(12, 47, 52)?);
    assert_eq!(Time::new(11, 11, 30)? + ((-33).hours() + 2.seconds()), Time::new(2, 11, 32)?);
    ...
    assert_eq!(Time::new(18, 3, 35)? - ((-85).minutes() + (-90).hours()), Time::new(13, 28, 35)?);
    assert_eq!(Time::new(2, 17, 44)? + 93.hours(), Time::new(23, 17, 44)?);
    assert_eq!(Time::new(8, 23, 42)? + ((-2).hours() + (-42).seconds()), Time::new(6, 23, 0)?);
    assert_eq!(Time::new(15, 44, 3)? - ((-7).minutes() + 62.hours()), Time::new(1, 51, 3)?);
    assert_eq!(Time::new(8, 48, 28)? - (51.minutes() + (-77).seconds()), Time::new(7, 58, 45)?);
    assert_eq!(Time::new(6, 7, 49)? - (50.minutes() + (-12).hours()), Time::new(17, 17, 49)?);
    assert_eq!(Time::new(3, 24, 42)? + (80.hours() + (-4).minutes()), Time::new(11, 20, 42)?);
    assert_eq!(Time::new(13, 21, 23)? + (-33).minutes(), Time::new(12, 48, 23)?);
    assert_eq!(Time::new(8, 23, 54)? + ((-63).seconds() + 56.hours()), Time::new(16, 22, 51)?);
    assert_eq!(Time::new(20, 26, 47)? + ((-50).seconds() + 53.hours()), Time::new(1, 25, 57)?);
    assert_eq!(Time::new(11, 50, 19)? - (36.hours() + (-30).minutes()), Time::new(0, 20, 19)?);
    Ok(())
}
